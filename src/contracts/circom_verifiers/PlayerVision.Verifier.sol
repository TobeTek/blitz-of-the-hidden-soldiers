...
contract PlayerVisionPlonkVerifier {
    // Omega
    ...
    uint16 constant nPublic   = 65;
    uint16 constant nLagrange = 65;
    
    uint256 constant Qmx  = 11823812888543791006480478388580910420486160637780557686899422107115757667672;
    uint256 constant Qmy  = 13196983264328621468614844569729746380728359393771493083149338365130372690544;
    uint256 constant Qlx  = 403723471200284798850030584613459489566236552536919595187914094445733362031;
    uint256 constant Qly  = 11980713963420136132976102927584668978942191812440486841897515145142125698485;
    uint256 constant Qrx  = 8209816366285797299283214144492751641941907664764273203627524123845285502418;
    uint256 constant Qry  = 18225750316111240457682166820733779642688606815487882432644206712134095250978;
    uint256 constant Qox  = 4277266953023715200614220767740647388738393964976241520342583978344091245583;
    uint256 constant Qoy  = 21259671615792621944541129525725853602057339149430278728036432642410922617281;
    uint256 constant Qcx  = 7292282370482857093940332215113756527698059073938355644666603612776142278408;
    uint256 constant Qcy  = 9806158554367033832782491852804545613267281458996162609929074180328593679231;
    uint256 constant S1x  = 407321643040727219777336782261468146642494030486371423346249787797440748319;
    uint256 constant S1y  = 16776848906257913334572214580239389350269036355485172222281135790241908458555;
    uint256 constant S2x  = 15532569680933950795823973529461972377906147643917623960804479638663341673329;
    uint256 constant S2y  = 10919350331087557461375792371350735899480071126852752054455875589272587049511;
    uint256 constant S3x  = 12936819232890117544834810157075580575187695574394827313464431681227731843138;
    uint256 constant S3y  = 15528449601718746364150321026708779094257847144034621919028433510589421357047;
    ...
    uint16 constant lastMem = 2880;

    function verifyProof(uint256[24] calldata _proof, uint256[65] calldata _pubSignals) public view returns (bool) {
        assembly {
            /////////
            ...
            function calculateChallenges(pMem, pPublic) {
                ...
                mstore(add(mIn, 2592 ), calldataload(pA))
                mstore(add(mIn, 2624 ), calldataload(add(pA, 32)))
                mstore(add(mIn, 2656 ), calldataload(pB))
                mstore(add(mIn, 2688 ), calldataload(add(pB, 32)))
                mstore(add(mIn, 2720 ), calldataload(pC))
                mstore(add(mIn, 2752 ), calldataload(add(pC, 32)))
                
                beta := mod(keccak256(mIn, 2784), q) 
            ...
            function calculateLagrange(pMem) {
                ...
                inverseArray(add(pMem, pZhInv), 66 )
